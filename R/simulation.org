...
#+TITLE: Literate SVEIRD Simulation (optionally with BDA)
#+AUTHOR: Bryce Carson
...
*** The function body
The body of the function is documented at a higher-level of sectioning, given
its length. See [[Simulation (optionally with data assimilation) cont.]].

** Examples
See the section [[Broken Examples]] in [[Miscellany]].

* Simulation (optionally with data assimilation) cont.
A modified SEI-type simulation is implemented in this section.

** The main loop
The whole SEI-type simulation is implemented with a single loop, which
continually overwrites components of an object named ~stack$rasterStack~
(falsely implying it is a RasterStack). The containing ~stack~ object has many
redundancies which are neither required to exist for the simulation, and which
are unused regardless.

The setup before the main loop of the simulation is simple enough, the component
SEI-type compartments of ~stack$rasterStack~ are "unpacked" and a summary data
frame is created to store the results of each iteration. An important object
named ~p~ is created, and the inital state of the simulation is "seeded".
...
[[https://pro.arcgis.com/en/pro-app/latest/tool-reference/spatial-analyst/how-focal-statistics-works.htm][ArcGIS
has a /perfect/ article examining focal statistics (including /literal edge
cases/) in their online tool reference.]] /terra/ performs calculations using
...
** write values into the summary table
...
* TODO =distwtRaster.R= The Euclidian distance weight matrix function
#+begin_comment
Veryify the next paragraph is accurate in its claims.
#+end_comment

The file =distwtRaster.R= defines a function ~wtd_nbrs_sum~, which calculates a
matrix of values which are weighted sums of values at every cell.

#+begin_quote
This code sets the Euclidean distance and the weight matrix
#+end_quote

The above quote is the comment on the file when in in [[source dependencies]]. It
doesn't make much sense to me, the comment, because it does not "set" a
Euclidian distance nor does it "set" a weighted sum in any one variable. It
returns a matrix of values which are weighted sums of values of functions of the
Euclidean distance from a given cell to another cell.

#+begin_comment
Again, verify the above paragraph is accurate in its claims.
#+end_comment

The full source of that file is reproduced here for discussion and analysis.
...
* TODO =rasterPlot.R= Visualization of rasters
...
* An R session loading the new code package for use with comparisons
